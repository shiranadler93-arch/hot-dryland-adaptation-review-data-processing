"""
Data loading and pre-processing for the solution-effectiveness model.

Contains DataProcessor, which turns a raw Excel export (one row per
combination of solutions, with a resulting effectiveness classification)
into the masked combination matrix and label arrays consumed by
`solution_efficiency_scoring.logit_model`.

This module has no top-level execution code -- import it and drive it from
a script (see result_reproduction.py).
"""
...
class DataProcessor:
    ...
    def import_data(self):
        ...
        self.raw_data["classification by result"] = self.raw_data[
    ...
    def build_combination_matrix(self):
        ...
        for comb in range(self.raw_data.shape[0]):
            sol_list = [
                x.strip()
                for x in [str(x) for x in self.raw_data.iloc[comb][:-2].to_list()]
                if x != "nan"
            ]
            self.data_dict[comb] = {
                "solutions": sol_list,
    ...
    def apply_isolation_mask(self):
        ...
        self.isolation_mask_tracking = pd.DataFrame(
            data={
                "suspected combination index": np.where(
                    self.comb_mat[:, (self.comb_mat.sum(axis=0) == 1)].sum(axis=1) > 0
                )[0]
            }
        )
        ...
        single_comb_participation = self.comb_mat[
            self.comb_mat[:, single_comb_bool].sum(axis=1) > 0, :
        ]
        combinations_w_singular_solutions = (
            single_comb_participation.sum(axis=1)
            - single_comb_participation[:, single_comb_bool].sum(axis=1)
        ) == 0

        self.isolation_mask_tracking["isolated combination"] = (
            combinations_w_singular_solutions.astype(int)
        )
        self.comb_mask_list = self.isolation_mask_tracking[
            self.isolation_mask_tracking["isolated combination"] == 1
        ]["suspected combination index"].to_list()

        # Track solution and combination lists
        self.isolation_mask_tracking["solution list"] = self.isolation_mask_tracking[
            "suspected combination index"
        ].apply(lambda x: self.data_dict[x]["solutions"])
        self.isolation_mask_tracking["solution index list"] = (
            self.isolation_mask_tracking["solution list"].apply(
                lambda x: [self.indexing_dict[sol] for sol in x]
            )
        )

        # Remaining combinations after masking singular combinations
        self.remaining_comb = [
            i for i in range(self.comb_mat.shape[0]) if i not in self.comb_mask_list
        ]

        # Remaining solutions after masking singular solutions
        self.sol_mask_list = []
        for comb in self.isolation_mask_tracking[
            self.isolation_mask_tracking["isolated combination"] == 1
        ]["solution index list"].to_list():
            for sol in comb:
                if sol not in self.sol_mask_list:
                    self.sol_mask_list.append(sol)
        self.remaining_sol = [
            i for i in range(self.comb_mat.shape[1]) if i not in self.sol_mask_list
        ]

        # Create masked indexing dictionaries
        self.masked_indexing_dict = {sol: i for i, sol in enumerate(self.remaining_sol)}
        self.masked_inv_indexing_dict = {
            i: sol for sol, i in self.masked_indexing_dict.items()
        }

        # Create masked combination matrix
        self.comb_mat_masked = self.comb_mat[self.remaining_comb, :][
            :, self.remaining_sol
        ]
        ...
        self.Y_masked = np.array(
            [
                self.data_dict[comb]["score"]
                for comb in self.data_dict.keys()
                if comb in self.remaining_comb
            ]
        )
        self.Y_masked_labeled = np.array(
            [self.label_dict[score] for score in self.Y_masked]
        )
    ...
    def update_labels_and_scores(self, conversion_instructions):
        ...
        This method updates Y, Y_masked, Y_masked_labeled and label_dict without
        modifying raw_data.
    ...
    def save_to_excel(self, output_prefix="logreg1"):
        ...
        train_df.to_excel(writer, sheet_name="train_data")

        # Raw data
        self.raw_data.to_excel(writer, sheet_name="raw_data")

        # Solution indexing
        pd.DataFrame(data={k: [v] for k, v in self.indexing_dict.items()}).to_excel(
            writer, sheet_name="solution_indexing"
        )

        # Solution mask list
        pd.DataFrame(
            data={self.inverse_indexing_dict[x]: [x] for x in self.sol_mask_list}
        ).to_excel(writer, sheet_name="solution_mask_list")

        # Combination mask tracking
        self.isolation_mask_tracking.to_excel(
            writer, sheet_name="combination_mask_tracking"
        )
